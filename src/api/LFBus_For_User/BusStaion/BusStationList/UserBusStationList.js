  ...
  Query: {
    UserBusStationList: async (_, args) => {
      ...
      const [results] = await connection.query(
        `SELECT BUS_NODE_ID, BUSSTOP_NM, GPS_LATI, GPS_LONG, distance_between(GPS_LATI, GPS_LONG, ${latitude}, ${longitude}) AS DISTANCE FROM BusStation WHERE distance_between(GPS_LATI, GPS_LONG, ${latitude}, ${longitude}) <= 0.5 ORDER BY DISTANCE DESC `
      );
      connection.release();
